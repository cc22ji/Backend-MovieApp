...
const {HomePageData,MoviePageData,TvPageData} = require("../models/ApiData")
        ...
        if(name && email && password && confirm_password){
            if(password === confirm_password){
               try {
                ...
                res.status(200).send({"status":"Success", "message":"Successfully Register"})
               } catch (error) {
                ...
                try {
                    ...
                    if (user) {
                       ...
                       if(!existingBookmark){
                        ...
                        await user.save();
                        res.json({ message: 'Movie bookmarked successfully', bookmarks: user.bookmarksId });
                       }else{
